#define semiLepBG_analysis_cxx
// The class definition in semiLepBG_analysis.h has been generated automatically
...
//
// root> T->Process("semiLepBG_analysis.C")
// root> T->Process("semiLepBG_analysis.C","some options")
// root> T->Process("semiLepBG_analysis.C+")
//


#include "semiLepBG_analysis.h"
#include <TH2.h>
#include <TStyle.h>

void semiLepBG_analysis::Begin(TTree * /*tree*/)
{
...
void semiLepBG_analysis::SlaveBegin(TTree * /*tree*/)
{
...
Bool_t semiLepBG_analysis::Process(Long64_t entry)
{
...
void semiLepBG_analysis::SlaveTerminate()
{
...
void semiLepBG_analysis::Terminate()
{
   ...
   std::ofstream file;
   file.open("semiLepBG_analysis.txt", std::ios::app); // file aperto in "append mode" per poter scrivere senza cancellare
   file << inf0 << " " << sup0 << " " << (sel0J*L*XS_semiLep)/j << " " << (sel0jj*L*XS_semiLep)/j << endl;
   file << inf1 << " " << sup1 << " " << (sel1J*L*XS_semiLep)/j << " " << (sel1jj*L*XS_semiLep)/j << endl;
   file << inf2 << " " << sup2 << " " << (sel2J*L*XS_semiLep)/j << " " << (sel2jj*L*XS_semiLep)/j << endl;
   file << inf3 << " " << sup3 << " " << (sel3J*L*XS_semiLep)/j << " " << (sel3jj*L*XS_semiLep)/j << endl;
   file << inf4 << " " << sup4 << " " << (sel4J*L*XS_semiLep)/j << " " << (sel4jj*L*XS_semiLep)/j << endl;
   file << inf5 << " " << sup5 << " " << (sel5J*L*XS_semiLep)/j << " " << (sel5jj*L*XS_semiLep)/j << endl;
   file << inf6 << " " << sup6 << " " << (sel6J*L*XS_semiLep)/j << " " << (sel6jj*L*XS_semiLep)/j << endl;
   file << inf7 << " " << sup7 << " " << (sel7J*L*XS_semiLep)/j << " " << (sel7jj*L*XS_semiLep)/j << endl;
   file << inf8 << " " << sup8 << " " << (sel8J*L*XS_semiLep)/j << " " << (sel8jj*L*XS_semiLep)/j << endl;
   file << inf9 << " " << sup9 << " " << (sel9J*L*XS_semiLep)/j << " " << (sel9jj*L*XS_semiLep)/j << endl;
   file.close();

   // scriviamo i dati senza categorizzazione J e jj in un altro file (T sta per totale)
   std::ofstream file_T;
   file_T.open("semiLepBG_analysis_T.txt", std::ios::app); // file aperto in "append mode" per poter scrivere senza cancellare
   file_T << inf0 << " " << sup0 << " " << sel0 << " " << (sel0*L*XS_semiLep)/j << endl;
   file_T << inf1 << " " << sup1 << " " << sel1 << " " << (sel1*L*XS_semiLep)/j << endl;
   file_T << inf2 << " " << sup2 << " " << sel2 << " " << (sel2*L*XS_semiLep)/j << endl;
   file_T << inf3 << " " << sup3 << " " << sel3 << " " << (sel3*L*XS_semiLep)/j << endl;
   file_T << inf4 << " " << sup4 << " " << sel4 << " " << (sel4*L*XS_semiLep)/j << endl;
   file_T << inf5 << " " << sup5 << " " << sel5 << " " << (sel5*L*XS_semiLep)/j << endl;
   file_T << inf6 << " " << sup6 << " " << sel6 << " " << (sel6*L*XS_semiLep)/j << endl;
   file_T << inf7 << " " << sup7 << " " << sel7 << " " << (sel7*L*XS_semiLep)/j << endl;
   file_T << inf8 << " " << sup8 << " " << sel8 << " " << (sel8*L*XS_semiLep)/j << endl;
   file_T << inf9 << " " << sup9 << " " << sel9 << " " << (sel9*L*XS_semiLep)/j << endl;
   file_T.close();

   cout << "\n\n\n***********************************************************************************\n" << endl;

   // vediamo se ha fatto tutti i cicli correttamnete o se si è perso qualcosa
   cout << j << "\tevents read (should be the number of events)" << endl;
   cout << k << "\titerations done (should be the number of events *N)\n" << endl;

   cout << "\ninf=" << inf0 << "\tsup=" << sup0 << "\t#sel=" << sel0 << "\tS=" << (sel0*L*XS_semiLep)/j << endl;
   cout << "inf=" << inf1 << "\tsup=" << sup1 << "\t#sel=" << sel1 << "\tS=" << (sel1*L*XS_semiLep)/j << endl;
   cout << "inf=" << inf2 << "\tsup=" << sup2 << "\t#sel=" << sel2 << "\tS=" << (sel2*L*XS_semiLep)/j<< endl;
   cout << "inf=" << inf3 << "\tsup=" << sup3 << "\t#sel=" << sel3 << "\tS=" << (sel3*L*XS_semiLep)/j<< endl;
   cout << "inf=" << inf4 << "\tsup=" << sup4 << "\t#sel=" << sel4 << "\tS=" << (sel4*L*XS_semiLep)/j<< endl;
   cout << "inf=" << inf5 << "\tsup=" << sup5 << "\t#sel=" << sel5 << "\tS=" << (sel5*L*XS_semiLep)/j<< endl;
   cout << "inf=" << inf6 << "\tsup=" << sup6 << "\t#sel=" << sel6 << "\tS=" << (sel6*L*XS_semiLep)/j<< endl;
   cout << "inf=" << inf7 << "\tsup=" << sup7 << "\t#sel=" << sel7 << "\tS=" << (sel7*L*XS_semiLep)/j<< endl;
   cout << "inf=" << inf8 << "\tsup=" << sup8 << "\t#sel=" << sel8 << "\tS=" << (sel8*L*XS_semiLep)/j<< endl;
   cout << "inf=" << inf9 << "\tsup=" << sup9 << "\t#sel=" << sel9 << "\tS=" << (sel9*L*XS_semiLep)/j<< endl;
